...
namespace CombatAI
{
	public class SightTracker : MapComponent
	{
		...
		public class SightReader
		{
			...
			public SightTracker Tacker => tacker;
			public Map Map => map;

			public SightReader(SightTracker tracker, ITSignalGrid[] friendlies, ITSignalGrid[] hostiles,
				ITSignalGrid[] neutrals)
			{
				tacker = tracker;
				map = tracker.map;
			...
			public float GetThreat(int index)
			{
				...
				{
					var attributes = GetMetaAttributes(index);
					if ((attributes & armor.weaknessAttributes) != MetaCombatAttribute.None) return 2.0f;
				}

				if (!Mod_CE.active)
					return armor.createdAt != 0
						? Mathf.Clamp01(2f * Maths.Max(GetBlunt(index) / (armor.Blunt + 0.001f),
							GetSharp(index) / (armor.Sharp + 0.001f), 0f))
						: 0f;
				else
					return armor.createdAt != 0
						? Mathf.Clamp01(Maths.Max(GetBlunt(index) / (armor.Blunt + 0.001f),
							GetSharp(index) / (armor.Sharp + 0.001f), 0f))
						: 0f;
			}
			...
			{
				var value = 0f;
				for (var i = 0; i < hostiles.Length; i++) value += hostiles[i].GetBlunt(index);
				return value;
			}
			...
			{
				var value = 0f;
				for (var i = 0; i < hostiles.Length; i++) value += hostiles[i].GetSharp(index);
				return value;
			}
			...
			{
				var value = MetaCombatAttribute.None;
				for (var i = 0; i < hostiles.Length; i++) value |= hostiles[i].GetCombatAttributesAt(index);
				return value;
			}
			...
			{
				var value = 0f;
				for (var i = 0; i < neutrals.Length; i++) value += neutrals[i].GetSignalNum(index);
				return value;
			}
			...
			{
				var value = 0f;
				for (var i = 0; i < hostiles.Length; i++) value += hostiles[i].GetSignalNum(index);
				return value;
			}
			...
			{
				var value = 0f;
				for (var i = 0; i < friendlies.Length; i++) value += friendlies[i].GetSignalNum(index);
				return value;
			}
			...
			{
				var value = 0f;
				for (var i = 0; i < neutrals.Length; i++) value += neutrals[i].GetSignalStrengthAt(index);
				return value;
			}
			...
			{
				var value = 0f;
				for (var i = 0; i < hostiles.Length; i++) value += hostiles[i].GetSignalStrengthAt(index);
				return value;
			}
			...
			{
				var value = 0f;
				for (var i = 0; i < friendlies.Length; i++) value += friendlies[i].GetSignalStrengthAt(index);
				return value;
			}
			...
			{
				ulong value = 0;
				for (var i = 0; i < hostiles.Length; i++) value |= hostiles[i].GetFlagsAt(index);
				return value;
			}
			...
			{
				ulong value = 0;
				for (var i = 0; i < friendlies.Length; i++) value |= friendlies[i].GetFlagsAt(index);
				return value;
			}
			...
			public Vector2 GetEnemyDirection(int index)
			{
				...
				for (var i = 0; i < hostiles.Length; i++) value -= hostiles[i].GetSignalDirectionAt(index);
				return value;
			}
			...
			{
				var value = Vector2.zero;
				for (var i = 0; i < friendlies.Length; i++) value += friendlies[i].GetSignalDirectionAt(index);
				return value;
			}
		}
		...
		public override void MapComponentTick()
		{
			...
			// --------------
			if (!Finder.Performance.TpsCriticallyLow || ticks % 2 == 1) insectsAndMechs.SightGridTick();
			// --------------
			if (!Finder.Performance.TpsCriticallyLow || ticks % 3 == 2) wildlife.SightGridTick();
			//
			// debugging stuff.
			if ((Finder.Settings.Debug_DrawShadowCasts || Finder.Settings.Debug_DrawThreatCasts) &&
			    GenTicks.TicksGame % 15 == 0)
			{
				_drawnCells.Clear();
				if (!Find.Selector.SelectedPawns.NullOrEmpty())
				{
					foreach (var pawn in Find.Selector.SelectedPawns)
					{
						...
						TryGetReader(pawn, out var reader);
								...
								for (var j = center.z - 64; j < center.z + 64; j++)
								{
									...
									if (cell.InBounds(map) && !_drawnCells.Contains(cell))
									{
										...
										if (Finder.Settings.Debug_DrawThreatCasts)
										{
											...
											if (value > 0)
												map.debugDrawer.FlashCell(cell, Mathf.Clamp01(value / 4f),
													$"{Math.Round(value, 2)}", 15);
										...
										else if (Finder.Settings.Debug_DrawShadowCasts)
										{
											...
											if (value > 0)
												map.debugDrawer.FlashCell(cell, Mathf.Clamp01(value / 20f),
													$"{Math.Round(value, 2)}", 15);
										}
									}
								}
				...
				else if (Finder.Settings.Debug_DrawShadowCasts)
				{
					...
					if (center.InBounds(map))
						...
						for (var j = center.z - 64; j < center.z + 64; j++)
						{
							...
							{
								_drawnCells.Add(cell);
								var value = raidersAndHostiles.grid.GetSignalStrengthAt(cell, out var enemies1) +
								            colonistsAndFriendlies.grid.GetSignalStrengthAt(cell, out var enemies2) +
								            insectsAndMechs.grid.GetSignalStrengthAt(cell, out var enemies4);
								if (value > 0)
									map.debugDrawer.FlashCell(cell, Mathf.Clamp(value / 7f, 0.1f, 0.99f),
										$"{Math.Round(value, 3)} {enemies1 + enemies2}", 15);
							}
						}
				}

				if (_drawnCells.Count > 0) _drawnCells.Clear();
			}
		}
		...
		public override void MapComponentOnGUI()
		{
			...
			Widgets.DrawBoxSolid(new Rect(0, 0, 3, 3),
				colonistsAndFriendlies.FactionNum == 1 ? Color.blue :
				colonistsAndFriendlies.FactionNum > 1 ? Color.green : Color.yellow);
			if (Finder.Settings.Debug_DrawShadowCastsVectors)
			{
				var turretTracker = map.GetComponent<TurretTracker>();
				var center = UI.MouseMapPosition().ToIntVec3();
				if (center.InBounds(map))
					foreach (var cell in GenRadial.RadialCellsAround(center, 24, true))
					{
						float enemies = raidersAndHostiles.grid.GetSignalNum(cell) +
						                colonistsAndFriendlies.grid.GetSignalNum(cell);
						Vector3 dir =
							(colonistsAndFriendlies.grid.GetSignalDirectionAt(cell) +
							 raidersAndHostiles.grid.GetSignalDirectionAt(cell)).normalized * 3;
						if (cell.InBounds(map) && enemies > 0)
						{
							Vector2 direction = dir * 0.25f;
							var start = UI.MapToUIPosition(cell.ToVector3Shifted());
							var end = UI.MapToUIPosition(cell.ToVector3Shifted() +
		...
		public void Register(Thing thing)
		{
			...
			var comp = thing.GetComp_Fast<ThingComp_CombatAI>();
			if (comp != null)
			{
				TryGetReader(thing, out var reader);
		...
		public void DeRegister(Thing thing)
		{
			...
			// notify pawn comps.
			var comp = thing.GetComp_Fast<ThingComp_CombatAI>();
			if (comp != null) comp.Notify_SightReaderChanged(null);
		}
