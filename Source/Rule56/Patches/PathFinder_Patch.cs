...
namespace CombatAI.Patches
{
	public static class PathFinder_Patch
	{
		...
		private static class PathFinder_FindPath_Patch
		{
			...
			internal static bool Prefix(PathFinder __instance, ref PawnPath __result, IntVec3 start,
				LocalTargetInfo dest, ref TraverseParms traverseParms, PathEndMode peMode,
				ref PathFinderCostTuning tuning, out bool __state)
			{
				if (fallbackCall) return __state = true;
				dump = false;
				if (Finder.Settings.Pather_Enabled && (pawn = traverseParms.pawn) != null && pawn.Faction != null &&
				    (pawn.RaceProps.Humanlike || pawn.RaceProps.IsMechanoid || pawn.RaceProps.Insect))
				{
					...
					if (armor.createdAt != 0) multiplier = Maths.Max(multiplier, 1 - armor.TankInt, 0.25f);
					// retrive CE elements
                     ...
                     */
					if (dump = Finder.Settings.Debug_DebugDumpData && !isPlayer && sightReader != null &&
					           avoidanceReader != null && Finder.Settings.Debug && Prefs.DevMode)
					{
						//if (gridWriter == null)
						//{
						...
						//}
						//gridWriter.Clear();
						if (pathWriter == null) pathWriter = new DataWriter_Path("pathing_csv", "pather_1");
						pathWriter.Clear();
					}

					var miningSkill = pawn.skills?.GetSkill(SkillDefOf.Mining)?.Level ?? 0f;
					if (dig = Finder.Settings.Pather_KillboxKiller && !dump && comp != null &&
					          !comp.TookDamageRecently(360) && sightReader != null &&
					          sightReader.GetAbsVisibilityToEnemies(pawn.Position) == 0 && pawn.RaceProps.Humanlike &&
					          pawn.HostileTo(map.ParentFaction) &&
					          (pawn.mindState?.duty?.def == DutyDefOf.AssaultColony ||
					           pawn.mindState?.duty?.def == DutyDefOf.AssaultThing ||
					           pawn.mindState?.duty?.def == DutyDefOf.HuntEnemiesIndividual))
					{
						raiders = true;
						var parms = traverseParms;
						...
						if (tuning == null)
						{
							...
							tuning.costBlockedWallExtraForNaturalWalls =
								(int)Maths.Max(45 * Maths.Max(10 - miningSkill, 0), 45);
					...
					if (sightReader != null)
					{
						if (!Finder.Performance.TpsCriticallyLow)
							visibilityAtDest =
								Maths.Min(
									sightReader.GetVisibilityToEnemies(dest.Cell) *
									Finder.Settings.Pathfinding_DestWeight, 5);
						else
			...
			public static void Postfix(PathFinder __instance, ref PawnPath __result, bool __state, IntVec3 start,
				LocalTargetInfo dest, TraverseParms traverseParms, PathEndMode peMode, PathFinderCostTuning tuning)
			{
				...
				if (__state)
				{
					if (Finder.Settings.Pather_KillboxKiller && dig && __result != null &&
					    !__result.nodes.NullOrEmpty() && (pawn?.RaceProps.Humanlike ?? false))
					{
						blocked.Clear();
						Thing blocker;
						if (__result.TryGetSapperSubPath(pawn, blocked, 15, 3, out var cellBefore, out var enemiesAhead,
							    out var enemiesBefore) && blocked.Count > 0 &&
						    (blocker = blocked[0].GetEdifice(map)) != null &&
						    pawn.mindState?.duty?.def != DutyDefOf.Sapper && pawn.CurJob?.def != JobDefOf.Mine)
						{
							if (tuning != null && (!enemiesAhead || enemiesBefore))
							{
								try
								{
									...
									tuning.costBlockedWallExtraForNaturalWalls =
										Maths.Max(tuning.costBlockedWallExtraForNaturalWalls * 3, 128);
									tuning.costBlockedWallExtraPerHitPoint =
										Maths.Max(tuning.costBlockedWallExtraPerHitPoint * 4, 4);
									__result = __instance.FindPath(start, dest, original_traverseParms, origina_peMode,
										tuning);
								}
								catch (Exception er)
								{
							...
							else
							{
								...
								if (job != null)
								{
									...
									{
										var count = 0;
										var countTarget = Rand.Int % 6 + 4 + Maths.Min(blocked.Count, 10);
										var faction = pawn.Faction;
										Predicate<Thing> validator = (t) =>
										{
											if (count < countTarget && t.Faction == faction && t is Pawn ally &&
											    !ally.Destroyed
											    && !ally.CurJobDef.Is(JobDefOf.Mine)
											    && ally.mindState?.duty?.def != DutyDefOf.Escort
											    && (sightReader == null ||
											        sightReader.GetAbsVisibilityToEnemies(ally.Position) == 0)
											    && ally.skills?.GetSkill(SkillDefOf.Mining).Level < 10)
											{
												var comp = ally.GetComp_Fast<ThingComp_CombatAI>();
												if (comp?.duties != null &&
												    comp.duties?.Any(DutyDefOf.Escort) == false)
												{
													var custom = CustomDutyUtility.Escort(ally, pawn, 20, 100,
														300 * blocked.Count + Rand.Int % 1000, 0, true, null);
													if (custom != null)
													{
										...
										Verse.GenClosest.RegionwiseBFSWorker(pawn.Position, map,
											ThingRequest.ForGroup(ThingRequestGroup.Pawn), PathEndMode.InteractionCell,
											TraverseParms.For(pawn), validator, null, 1, 10, 40, out var _);
									}
								}
							}
						}
					...
					var tracker = pawn.Map.GetComp_Fast<AvoidanceTracker>();
					if (tracker != null) tracker.Notify_PathFound(pawn, __result);
				}
				...
				ILGenerator generator)
			{
				...
				{
					if (!finished1)
						if (codes[i].opcode == OpCodes.Stloc_S && codes[i].operand is LocalBuilder builder1 &&
						    builder1.LocalIndex == 48)
						{
							finished1 = true;
							yield return new CodeInstruction(OpCodes.Ldloc_S, 45).MoveLabelsFrom(codes[i])
								.MoveBlocksFrom(codes[i]); // index of cell around curIndex
							...
							yield return new CodeInstruction(OpCodes.Call,
								AccessTools.Method(typeof(PathFinder_FindPath_Patch), nameof(GetCostOffsetAt)));
				...
				if (map != null)
				{
					...
					if (sightReader != null)
					{
						...
						if (visibility > visibilityAtDest)
						{
							...
							if (threat >= threatAtDest) value += (int)(threat * 64f);
						}

						if (!isPlayer)
						{
							var attributes = sightReader.GetMetaAttributes(index);
							if ((attributes & MetaCombatAttribute.AOELarge) != MetaCombatAttribute.None) path = 2f;
						}
					}
					...
					if (avoidanceReader != null && !isPlayer)
					{
						...
						var danger = avoidanceReader.GetDanger(index);
						if (danger > 0) value += 23;
					...
					if (value > 10f)
					{
						...
						var l1 = 350 * (1f - Mathf.Lerp(0f, 0.75f, counter / (openNum + 1f))) *
						         (1f - Maths.Min(openNum, 5000) / 7500);
