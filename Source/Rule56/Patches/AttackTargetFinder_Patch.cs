		...
		internal static class AttackTargetFinder_BestAttackTarget_Patch
		{
			internal static void Prefix(IAttackTargetSearcher searcher)
			{
				...
				if (searcher.Thing is Pawn pawn && !pawn.RaceProps.Animal)
				{
					...
					if (searcherVerb != null && !searcherVerb.IsMeleeAttack &&
					    (projectile = searcherVerb.GetProjectile()?.projectile ?? null) != null)
		...
		internal static class Harmony_AttackTargetFinder_GetShootingTargetScore
		{
					...
					if (!finished)
						if (codes[i].opcode == OpCodes.Ldc_R4)
						{
							...
							yield return new CodeInstruction(OpCodes.Call,
								AccessTools.Method(typeof(Harmony_AttackTargetFinder_GetShootingTargetScore),
									nameof(GetTargetBaseScore)));
							continue;
						}
			...
			public static float GetTargetBaseScore(IAttackTarget target, IAttackTargetSearcher searcher, Verb verb)
			{
				...
				if (sightReader != null && searcherPawn != null)
				{
					...
					{
						if (sightReader.GetAbsVisibilityToEnemies(target.Thing.Position) >
						    sightReader.GetAbsVisibilityToFriendlies(target.Thing.Position) + 1)
							result -= 30f * Finder.P50;
						if (sightReader.GetVisibilityToEnemies(target.Thing.Position) > 3) result -= 15f * Finder.P50;
						result += sightReader.GetEnemyDirection(target.Thing.Position).sqrMagnitude -
						          Mathf.Pow(sightReader.GetVisibilityToEnemies(target.Thing.Position), 2);
					}
					...
					if (target.Thing is Pawn enemy)
					{
						if (damageReport.IsValid)
						{
							...
							var diff = 0f;
							if (Mod_CE.active)
								diff = Mathf.Clamp01(Maths.Max(damageReport.adjustedBlunt / (armorReport.Blunt + 1f),
									damageReport.adjustedSharp / (armorReport.Sharp + 1f), 0f));
							else
								diff = Mathf.Clamp01(1 - Maths.Max(armorReport.Blunt - damageReport.adjustedBlunt,
									armorReport.Sharp - damageReport.adjustedSharp, 0f));
							result += 8f * diff;
						...
						Thing targeted;
						if (enemy.stances?.stagger != null && enemy.stances.stagger.Staggered) result += 12;
						if (!verb.IsMeleeAttack)
						{
							if (enemy.CurrentEffectiveVerb?.IsMeleeAttack ?? false)
								if (searcherFaction != null &&
								    (targeted = enemy.jobs?.curJob?.targetA.Thing)?.Faction == searcherFaction)
									result += 8 + enemy.Position.DistanceToSquared(targeted.Position) < 150 ? 16 : 0;
							if (enemy.pather?.MovingNow ?? false) result += 16;
						}

						if (!enemy.HasWeaponVisible()) result -= 8;
					}
				}
