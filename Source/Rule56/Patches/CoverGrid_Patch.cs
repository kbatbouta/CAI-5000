...
namespace CombatAI.Patches
{
	public static class CoverGrid_Patch
	{
		...
		public static MethodBase mCellToIndex2 = AccessTools.Method(typeof(CellIndices),
			nameof(CellIndices.CellToIndex), new[] { typeof(int), typeof(int) });
		...
		public static class CoverGrid_Register_Patch
		{
			...
			{
				grid = t.def.fillPercent > 0 ? __instance.map.GetComp_Fast<WallGrid>() : null;
			}
			...
			public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
			{
				...
				for (var i = 0; i < codes.Count; i++)
				{
					if (!finished)
						...
						{
							finished = true;
							yield return new CodeInstruction(OpCodes.Ldarg_1).MoveLabelsFrom(codes[i])
								.MoveBlocksFrom(codes[i]);
							yield return new CodeInstruction(OpCodes.Ldloc_0);
							yield return new CodeInstruction(OpCodes.Call,
								AccessTools.Method(typeof(CoverGrid_Patch), nameof(Set)));
						}
		...
		{
			if (grid != null) grid.RecalculateCell(cell, t);
		}
	}
}