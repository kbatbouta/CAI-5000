...
namespace CombatAI.Patches
{
	public static class GenSight_Patch
	{
		...
		[HarmonyPatch(typeof(GenSight), nameof(GenSight.LineOfSight),
			new[]
			{
				typeof(IntVec3), typeof(IntVec3), typeof(Map), typeof(bool), typeof(Func<IntVec3, bool>), typeof(int),
				typeof(int)
			})]
		private static class GenSight_LineOfSight1_Patch
		{
			public static void Prefix(IntVec3 start, IntVec3 end, Map map, bool skipFirstCell,
				ref Func<IntVec3, bool> validator, int halfXOffset, int halfZOffset)
			{
		...
		{
			public static void Prefix(IntVec3 start, IntVec3 end, Map map, CellRect startRect, CellRect endRect,
				ref Func<IntVec3, bool> validator)
			{
				if (GenSight_Patch.map != map)
				{
					GenSight_Patch.map = map;
