...
namespace CombatAI.Patches
{
	public static class Harmony_CastPositionFinder
	{
		...
		{
			private static FieldInfo fBestSpotPref =
				AccessTools.Field(typeof(CastPositionFinder), nameof(CastPositionFinder.bestSpotPref));
			...
			public static void Prefix(ref CastPositionRequest newReq)
			{
				...
				skipped = true;
				if (newReq.caster != null && newReq.target != null &&
				    (newReq.maxRangeFromTarget == 0 || newReq.maxRangeFromTarget * newReq.maxRangeFromTarget >
					    newReq.caster.Position.DistanceToSquared(newReq.target.Position)) &&
				    newReq.maxRangeFromLocus == 0)
					if ((pawn = newReq.caster) != null && !(pawn.RaceProps?.Animal ?? true) &&
					    !((pawn.Faction?.IsPlayerSafe() ?? false) && !pawn.Drafted && pawn.mindState?.duty == null) &&
					    !(pawn.mindState != null && (pawn.mindState.duty?.def == DutyDefOf.Sapper ||
			...
			{
				if (__result && avoidanceTracker != null) avoidanceTracker.Notify_CoverPositionSelected(pawn, dest);
					...
					if (!finished)
						if (codes[i].OperandIs(fBestSpotPref))
						{
							...
							yield return new CodeInstruction(OpCodes.Call,
								AccessTools.Method(typeof(CastPositionFinder_TryFindCastPosition_Patch),
									nameof(FloodCellRect)));
						}
				}
			}
			...
			private static void FloodCellRect(CellRect rect)
			{
				if (!skipped && sightReader != null)
				{
					...
					var rootVis = sightReader.GetVisibilityToEnemies(root);
					...
					map.GetCellFlooder().Flood(root,
						(node) =>
						{
							grid[node.cell] = (node.dist - node.distAbs) / (node.distAbs + 1f) +
							                  (sightReader.GetVisibilityToEnemies(node.cell) - rootVis) * 0.25f +
							                  Maths.Min(avoidanceReader.GetProximity(node.cell), 4f) -
							                  Maths.Min(avoidanceReader.GetDanger(node.cell), 1f) -
							                  interceptors.grid.Get(node.cell) * 4 +
							                  (sightReader.GetThreat(node.cell) - rootThreat) * 0.5f;
						},
						(cell) =>
						{
							...
							//}
							return (sightReader.GetVisibilityToEnemies(cell) - rootVis) * 2 -
								interceptors.grid.Get(cell) + (sightReader.GetThreat(cell) - rootThreat) * 0.25f;
						},
						(cell) =>
						{
							return rect.Contains(cell) && cell.WalkableBy(map, pawn) &&
