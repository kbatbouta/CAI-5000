	...
	public class AvoidanceTracker : MapComponent
	{
		...
		public override void MapComponentTick()
		{
			...
			for (var i = 0; i < items.Count; i++)
			{
				...
				if (item.pawn.Downed || GenTicks.TicksGame - (item.pawn.needs?.rest?.lastRestTick ?? 0) < 30) continue;
				TryCastProximity(item.pawn, IntVec3.Invalid);
				if (item.pawn.pather?.MovingNow ?? false) TryCastPath(item.pawn, null);
			}

			for (var i = 0; i < _removalList.Count; i++) DeRegister(_removalList[i]);
			if (buckets.Index == 0)
			{
				wait = true;
					...
					if (center.InBounds(map))
						...
						for (var j = center.z - 64; j < center.z + 64; j++)
						{
							...
							if (cell.InBounds(map))
							{
								...
								if (value > 0)
									map.debugDrawer.FlashCell(cell, Mathf.Clamp(value / 10f, 0f, 0.99f),
										$"{path.Get(cell)} {proximity.Get(cell)}", 15);
							}
						}
				}
				else
							...
							if (center.InBounds(map))
								...
								for (var j = center.z - 64; j < center.z + 64; j++)
								{
									...
									if (cell.InBounds(map) && !_drawnCells.Contains(cell))
									{
										...
										if (value > 0)
											map.debugDrawer.FlashCell(cell, Mathf.Clamp(value / 10f, 0f, 0.99f),
												$"{reader.GetPath(cell)} {reader.GetProximity(cell)}", 15);
									}
								}
						}
					}
				...
				if (center.InBounds(map))
					...
					for (var j = center.z - 64; j < center.z + 64; j++)
					{
						...
						if (cell.InBounds(map))
						{
							...
							if (value > 0)
								map.debugDrawer.FlashCell(cell, Mathf.Clamp(value / 32f, 0f, 0.99f),
									$"{Math.Round(affliction_pen.Get(cell), 1)} {Math.Round(affliction_dmg.Get(cell), 1)}",
									15);
						}
					}
			}
		...
		{
			if (buckets.ContainsId(pawn.thingIDNumber)) buckets.RemoveId(pawn.thingIDNumber);
			if (Valid(pawn)) buckets.Add(new IBucketablePawn(pawn, pawn.thingIDNumber % buckets.numBuckets));
		}
		...
		public void Notify_Injury(Pawn pawn, DamageInfo dinfo)
		{
			...
			asyncActions.EnqueueOffThreadAction(() =>
			{
				...
				{
					var f = Maths.Max(1 - node.dist / 5.65685424949f, 0.25f);
		...
		private void TryCastProximity(Pawn pawn, IntVec3 dest)
		{
			IntVec3 orig;
			orig = pawn.Position;
			var flags = pawn.GetThingFlags();
			if (pawn.pather?.MovingNow == true && (dest.IsValid || (dest = pawn.pather.Destination.Cell).IsValid))
				asyncActions.EnqueueOffThreadAction(() =>
				{
					proximity.Next();
					flooder.Flood(orig, (node) => { proximity.Set(node.cell, 1, flags); }, maxDist: 2);
					flooder.Flood(dest, (node) => { proximity.Set(node.cell, 1, flags); }, maxDist: 2);
				});
			else
				asyncActions.EnqueueOffThreadAction(() =>
				{
		...
		private void TryCastPath(Pawn pawn, PawnPath pawnPath = null)
		{
			...
			if (pawnPath?.nodes == null || pawnPath.curNodeIndex <= 5) return;
			var flags = pawn.GetThingFlags();
			var cells = pawnPath.nodes.GetRange(Maths.Max(pawnPath.curNodeIndex - 80, 0),
				Maths.Min(pawnPath.curNodeIndex + 1, 80));
			...
			asyncActions.EnqueueOffThreadAction(() =>
			{
				...
				for (var i = 1; i < cells.Count; i++)
				{
					...
					var dx = Math.Sign(prev.x - cur.x);
					var dz = Math.Sign(prev.z - cur.z);
					prev = cur;
					if (dx == 0)
					{
