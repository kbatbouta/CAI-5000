...
namespace CombatAI
{
	...
	public class Mod_CE
	{
		[Unsaved] private static readonly FlagArray turretsCE = new FlagArray(short.MaxValue);
		...
		[LoadNamed("CombatExtended.ToolCE")] public static Type ToolCE;
		...
		{
			return Verb_ShootCE_isAiming != null && Verb_ShootCE.IsInstanceOfType(verb) &&
			       (bool)Verb_ShootCE_isAiming.GetValue(verb);
		}
		...
		public static bool IsTurretActiveCE(Building_Turret turret)
		{
			...
			return turretsCE[turret.def.index] &&
			       (((manable = (bool)Building_TurretGunCE_IsMannable.Invoke(turret, Array.Empty<object>())) &&
			         (bool)Building_TurretGunCE_MannedByColonist.Invoke(turret, Array.Empty<object>())) ||
			        (!manable && (bool)Building_TurretGunCE_Active.Invoke(turret, Array.Empty<object>())));
		}

		public static float GetProjectileArmorPenetration(ProjectileProperties props)
		{
			if (props.GetType() != ProjectilePropertiesCE) return props.GetArmorPenetration(1);
		...
		private static void OnActive()
		{
			...
			Finder.Harmony.Patch(AccessTools.Method(Building_TurretGunCE, nameof(Building_Turret.SpawnSetup)),
				postfix: new HarmonyMethod(AccessTools.Method(typeof(Building_TurretGunCE_Patch),
					nameof(Building_TurretGunCE_Patch.SpawnSetup))));
			Finder.Harmony.Patch(AccessTools.Method(Building_TurretGunCE, nameof(Building_Turret.DeSpawn)),
				new HarmonyMethod(AccessTools.Method(typeof(Building_TurretGunCE_Patch),
					nameof(Building_TurretGunCE_Patch.DeSpawn))));
		}
