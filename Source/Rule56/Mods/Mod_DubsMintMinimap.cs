...
namespace CombatAI
{
	...
	public class Mod_DubsMintMinimap
	{
		...
		[Unsaved] private static MapComponent_FogGrid fogGrid;

		[LoadNamed("DubsMintMinimap.MainTabWindow_MiniMap:Fogged", LoadableType.Method, new Type[] { typeof(Thing) })]
		...
		{
			Finder.Harmony.Patch(DrawAllPawns,
				new HarmonyMethod(AccessTools.Method(typeof(Mod_DubsMintMinimap), nameof(PreDrawAllPawns))),
				new HarmonyMethod(AccessTools.Method(typeof(Mod_DubsMintMinimap), nameof(PostDrawAllPawns))));
			Finder.Harmony.Patch(Fogged,
				new HarmonyMethod(AccessTools.Method(typeof(Mod_DubsMintMinimap), nameof(IsFogged))));
		}
