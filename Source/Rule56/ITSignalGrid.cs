...
namespace CombatAI
{
	...
	public class ITSignalGrid
	{
		private struct IField<T> where T : struct
		{
			...
			{
				valuePrev = expired ? default : value;
				value = newVal;
			}
		}
		...
		public void Set(int index, float signalStrength, Vector2 dir)
		{
			...
			{
				var info = cells[index];
				if (info.sig != r_sig)
				{
					var dc = r_cycle - info.cycle;
					if (dc == 0)
					{
						info.num += 1;
		...
		public float GetRawSignalStrengthAt(int index)
		{
			if (index >= 0 && index < NumGridCells)
			{
				...
				{
					case 0:
						var strength = cells_strength[index];
			...
			if (index >= 0 && index < NumGridCells)
			{
				...
				switch (r_cycle - cell.cycle)
				{
					case 0:
						...
						return Maths.Max(strength.value, strength.valuePrev) * 0.9f +
						       Maths.Max(cell.num, cell.numPrev) * 0.1f;
					case 1:
		...
		public Vector2 GetSignalDirectionAt(int index)
		{
			if (index >= 0 && index < NumGridCells)
			{
				...
				{
					case 0:
						var dir = cells_dir[index];

						return cell.num >= cell.numPrev
							? dir.value / (cell.num + 0.01f)
							: dir.valuePrev / (cell.numPrev + 0.01f);
					case 1:
		...
		{
			if (r_sig++ == short.MaxValue) r_sig = 19;
			if (r_cycle++ == short.MaxValue) r_cycle = 13;
		}
	}
}