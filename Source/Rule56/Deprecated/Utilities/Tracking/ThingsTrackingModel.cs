...
namespace CombatAI.Utilities
{
	...
	public class ThingsTrackingModel
	{
		...
		public void Register(Thing thing)
		{
			...
			{
				var temp = new ThingPositionInfo[sortedThings.Length * 2];
		...
		{
			if (indexByThing.TryGetValue(thing, out var index))
			{
				RemoveClean(index);
		...
		public IEnumerable<Thing> ThingsNearSegment(IntVec3 origin, IntVec3 destination, float range, bool behind)
		{
			...
			if (lengthSq ==
			    0) // origin and destination cell are the same, so just return all things within range radius of origin.
			{
				foreach (var t in ThingsInRangeOf(origin, range)) yield return t;
				yield break;
			}
			...
			while (top != bottom && limiter++ < 20) // try to find a good starting point for iterating nearby pawns
			{
				...
				if (midPosition.x > minX && midPosition.x < maxX) break;
			...
			{
				var t = sortedThings[index++].thing;
				var curPosition = t.Position;
				// if we're outside the range of interest, we're done checking to the right.
				if (curPosition.x + range < minX || curPosition.x - range > maxX) break;

				var relativePosition = curPosition - origin;

				// Calculate the dot product of the relativePosition (rp) vector with itself (the square of its magnitude)
				float rp_rp_dot = relativePosition.x * relativePosition.x + relativePosition.y * relativePosition.y +
				...
				float rp_direction_dot = relativePosition.x * direction.x + relativePosition.y * direction.y +
				                         relativePosition.z * direction.z;

				// Calculate the ratio of the two dot products;
				var dot = rp_direction_dot / rp_rp_dot;
				if (dot > 1) // curPosition is beyond the end of the line segment.
				{
					if (destination.DistanceToSquared(curPosition) <= rangeSq) yield return t;
					continue;
				}

				if (dot < 0) // curPosition is behind the origin
				{
					if (behind && origin.DistanceToSquared(curPosition) <= rangeSq) yield return t;
					continue;
				}

				// dot ∈ (0,1) so curPosition is between the origin and destination
				var projection = new IntVec3((int)(origin.x + dot * direction.x),
					(int)(origin.y + dot * direction.y),
					(int)(origin.z + dot * direction.z));
				if (projection.DistanceToSquared(curPosition) <= rangeSq) yield return t;
			}
