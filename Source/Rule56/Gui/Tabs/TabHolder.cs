...
namespace CombatAI.Gui.Tabs
{
	public class TabHolder
	{
		...
		public void DoContent(Rect inRect)
		{
			...
			GUIUtility.ExecuteSafeGUIAction(() =>
			{
				...
				if (useSidebar)
				{
					...
					var tabsRect = inRect.LeftPartPixels(50);
					var contentRect = new Rect(inRect);
					contentRect.xMin += 60;
					GUIUtility.ExecuteSafeGUIAction(() => { DoSidebar_newtemp(tabsRect); });
					GUIUtility.ExecuteSafeGUIAction(() =>
					{
						GUIFont.Font = GUIFontSize.Small;
						GUIFont.CurFontStyle.fontStyle = FontStyle.Bold;
						GUIFont.Anchor = TextAnchor.MiddleLeft;
						var headerHeight = "RocketEngine.Gui".GetTextHeight(inRect.width) + 3;
						var rect = contentRect.TopPartPixels(headerHeight);
						// Create the RocketEngine.Gui stamp                        
						Widgets.Label(rect, "RocketEngine.Gui");
		...
		{
			var active = tabs.Count(t => t.ShouldShow);
			...
			foreach (var tab in tabs)
			{
				...
				if (tab.Selected) Widgets.DrawWindowBackgroundTutor(rect);
		...
		private void DoSidebar(Rect rect)
		{
			...
			foreach (var tab in tabs)
			{
				...
				if (tab.Selected) Widgets.DrawWindowBackgroundTutor(curRect);
