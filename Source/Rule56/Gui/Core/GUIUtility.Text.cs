...
using Verse.Sound;
using GUITextState =
	System.Tuple<string, CombatAI.Gui.GUIFontSize, System.Tuple<float, float>, System.Tuple<int, int, int, int>, System.
		Tuple<UnityEngine.FontStyle, UnityEngine.FontStyle, UnityEngine.FontStyle, UnityEngine.FontStyle>>;
...
namespace CombatAI.Gui
{
	public static partial class GUIUtility
	{
		...
		private static readonly Dictionary<GUITextState, float> textHeightCache =
			new Dictionary<GUITextState, float>(512);
		...
		{
			Cleanup();
			var height = GetTextHeight(text, rect.width);
		...
		{
			Cleanup();
			var key = GetGUIState(text, width);
			if (textHeightCache.TryGetValue(key, out var height)) return height;
