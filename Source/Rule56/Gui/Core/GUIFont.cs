...
namespace CombatAI.Gui
{
	...
	public static partial class GUIFont
	{
		...
		public static TextAnchor Anchor
		{
			get
			{
				if (UseCustomFonts) return anchorInt;
		...
		public static bool WordWrap
		{
			get
			{
				if (UseCustomFonts) return wordWrapInt;
				throw new Exception("Used GUIFont.CurFontStyle outside a safe scope");
			}
			set
			{
		...
		public static GUIFontSize Font
		{
			get
			{
				if (UseCustomFonts) return fontInt;
				throw new Exception("Used GUIFont.CurFontStyle outside a safe scope");
			}
			set
			{
		...
		public static GUIStyle CurTextAreaReadOnlyStyle
		{
			get
			{
				if (UseCustomFonts)
				{
					var style = temptextAreaReadOnlyStyles[0];
					switch (fontInt)
					{
						case GUIFontSize.Tiny:
		...
		public static GUIStyle CurTextAreaStyle
		{
			get
			{
				if (UseCustomFonts)
				{
					var style = temptextAreaStyles[0];
					switch (fontInt)
					{
						case GUIFontSize.Tiny:
		...
		public static GUIStyle CurTextFieldStyle
		{
			get
			{
				if (UseCustomFonts)
				{
					var style = temptextFieldStyles[0];
					switch (fontInt)
					{
						case GUIFontSize.Tiny:
		...
		static GUIFont()
		{
			...
			for (var i = 0; i < textFieldStyles.Length; i++)
			{
			...
			for (var j = 0; j < textAreaStyles.Length; j++)
			{
			...
			for (var k = 0; k < textAreaReadOnlyStyles.Length; k++)
			{
			...
			var index = 0;
			foreach (var value in new GUIFontSize[]
