...
namespace CombatAI
{
	public class PerformanceTracker : GameComponent
	{
		...
		public override void GameComponentTick()
		{
			...
			var deltaT = (float)stopwatch.ElapsedTicks / Stopwatch.Frequency;
			stopwatch.Restart();
			if (deltaT > 0.5f)
			{
				stopwatch.Stop();
		...
		public override void GameComponentOnGUI()
		{
			...
			{
				var lowMsg = TpsCriticallyLow ? "<color=red>LOW</color>" : "NROMAL";
				Widgets.DrawBoxSolid(new Rect(Vector2.zero, new Vector2(100, 5)), Color.gray);
				Widgets.DrawBoxSolid(new Rect(Vector2.zero, new Vector2(100 * Performance, 5)),
					TpsCriticallyLow ? Color.yellow : Color.blue);
				Widgets.Label(new Rect(Vector2.zero, new Vector2(100, 25)),
					$"{Tps}\t{lowMsg}\t{Math.Round(Performance, 2)}");
			}
		}
	}
}