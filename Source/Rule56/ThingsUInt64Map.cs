				...
				for (; i < m; i++)
				{
					if ((c & flag) != 0)
					{
						...
						for (var j = 0; j < 8; j++)
						{
							if ((flag & k) != 0)
							{
								var items = buckets.GetBucket(j + i * 8);
								for (var l = 0; l < items.Count; l++) yield return items[l].thing;
							}
				...
				for (; i < m; i++)
				{
					if ((c & flag) != 0)
					{
						...
						for (var j = 0; j < 8; j++)
						{
							...
							{
								var items = buckets.GetBucket(j + i * 8);
								for (var l = 0; l < items.Count; l++) bufferList.Add(items[l].thing);
							}
			...
			unchecked
			{
				...
				for (; i < m; i++)
				{
					if ((c & flag) != 0)
					{
						...
						for (var j = 0; j < 8; j++)
						{
							if ((flag & k) != 0) yield return j + i * 8;
							k = k << 1;
						}
					}
