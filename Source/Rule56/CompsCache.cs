...
namespace CombatAI
{
    public static class CompsCache
    {
        ...
        static Dictionary<int, List<Action<ThingWithComps>>> thingComps_removeActionsByThing = new Dictionary<int, List<Action<ThingWithComps>>>();

        static class ThingComp_Cache<T> where T : ThingComp
        {
            public readonly static Dictionary<int, T> compsById = new Dictionary<int, T>(1024);            
        ...
        static class MapComponent_Cache<T> where T : MapComponent
        {
        ...
        public static T GetComp_Fast<T>(this ThingWithComps thing, bool allowFallback = true) where T : ThingComp
        {
            ...
            {
                MethodInfo info = AccessTools.Method(typeof(ThingComp_Cache<>).MakeGenericType(new Type[] { typeof(T) }), "Remove");
                thingComps_removeActions.Add(typeof(T), removeAction = (t) =>
                {
                    info.Invoke(null, new[] { t });
                });
            }
        ...
        public static void Notify_MapRemoved(Map map)
        {
            if (mapComponents_removeActions == null)
            {
                ...
                {
                    MethodInfo info = AccessTools.Method(typeof(MapComponent_Cache<>).MakeGenericType(new Type[] { compType }), "Remove");
