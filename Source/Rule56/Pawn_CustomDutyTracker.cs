...
namespace CombatAI
{
	public class Pawn_CustomDutyTracker : IExposable
	{
		...
		public DutyDef CurDutyDef => curCustomDuty?.duty?.def ?? pawn.mindState?.duty?.def ?? null;
		...
		public void TickRare()
		{
			if (curCustomDuty != null)
			{
				...
				else if (curCustomDuty.failOnDistanceToFocus > 0 && (curCustomDuty.duty.focus.Cell.IsValid == false ||
				                                                     curCustomDuty.duty.focus.Cell.DistanceToSquared(
					                                                     pawn.Position) >
				                                                     curCustomDuty.failOnDistanceToFocus *
				                                                     curCustomDuty.failOnDistanceToFocus))
				{
					curCustomDuty = null;
					pawn.mindState.duty = null;
				...
				{
					var focus = curCustomDuty.duty.focus.Thing;
					...
					else if (curCustomDuty.failOnFocusDeath || curCustomDuty.failOnFocusDowned)
					{
						...
						else if (curCustomDuty.failOnFocusDutyNot != null &&
						         fpawn.mindState?.duty?.def != curCustomDuty.failOnFocusDutyNot)
						{
							curCustomDuty = null;
							pawn.mindState.duty = null;
						}
			...
			if (curCustomDuty == null && queue.Count > 0)
				...
				{
					var next = queue[0];
					if (next.startsAt > GenTicks.TicksGame) break;
			...
			if (curCustomDuty != null && pawn.mindState.duty != curCustomDuty.duty &&
			    !IsForcedDuty(pawn.mindState.duty?.def ?? null)) pawn.mindState.duty = curCustomDuty.duty;
		}

		public void StartDuty(CustomPawnDuty duty, bool returnCurDutyToQueue = true)
		{
			if (IsForcedDuty(pawn.mindState.duty?.def ?? null) ||
			    (curCustomDuty != null && IsForcedDuty(curCustomDuty.duty.def))) return;
			if (returnCurDutyToQueue)
			{
				...
				{
					var custom = new CustomPawnDuty();
					custom.duty = pawn.mindState.duty;
					EnqueueFirst(custom);
				}
			}

			if (duty.startAfter > 0) duty.startsAt = GenTicks.TicksGame + duty.startAfter;
			if (duty.expireAfter > 0) duty.expiresAt = GenTicks.TicksGame + duty.expireAfter;
			curCustomDuty = duty;
			if (pawn.mindState != null) pawn.mindState.duty = duty.duty;
		}
		...
		public void ExposeData()
		{
			...
			if (queue == null) queue = new List<CustomPawnDuty>();
		}

		private bool IsExitDuty(DutyDef def)
		{
			return def != null && (def == DutyDefOf.ExitMapBest || def == DutyDefOf.ExitMapRandom ||
			                       def == DutyDefOf.ExitMapNearDutyTarget ||
			                       def == DutyDefOf.ExitMapBestAndDefendSelf || def == DutyDefOf.TravelOrLeave ||
			                       def == DutyDefOf.TravelOrWait);
		}

		private bool IsForcedDuty(DutyDef def)
		{
			return def != null && (IsExitDuty(def) || def == DutyDefOf.PrisonerEscape ||
			                       def == DutyDefOf.PrisonerEscapeSapper || def == DutyDefOf.PrisonerAssaultColony ||
			                       def == DutyDefOf.Kidnap || def == DutyDefOf.Steal);
		}
	}
}