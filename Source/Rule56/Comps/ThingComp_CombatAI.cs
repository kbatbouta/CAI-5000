...
namespace CombatAI.Comps
{
	public class ThingComp_CombatAI : ThingComp
	{
		...
		public override void CompTickRare()
		{
			...
			if (duties != null) duties.TickRare();
		}

		public override void CompTickLong()
		{
			base.CompTickLong();
			if (parent is Pawn pawn) armor = ArmorUtility.GetArmorReport(pawn);
		}
		...
		public void OnScanStarted()
		{
			if (visibleEnemies.Count != 0)
			{
				...
				{
					Log.Warning(
						$"ISMA: OnScanStarted called while scanning. ({visibleEnemies.Count}, {Thread.CurrentThread.ManagedThreadId})");
					return;
				}
		...
		public void OnScanFinished()
		{
			...
			{
				Log.Warning(
					$"ISMA: OnScanFinished called while not scanning. ({visibleEnemies.Count}, {Thread.CurrentThread.ManagedThreadId})");
				return;
			}
			...
			{
				var bodySize = pawn.RaceProps.baseBodySize;
				// pawn reaction cooldown changes with their bodysize.
				if (GenTicks.TicksGame - lastInterupted < 60 * bodySize ||
				    GenTicks.TicksGame - lastRetreated < 65 * bodySize) return;
				// if CE is acrive skip reaction if the pawn is reloading or hunkering down.
				if (Mod_CE.active &&
				    (pawn.CurJobDef.Is(Mod_CE.ReloadWeapon) || pawn.CurJobDef.Is(Mod_CE.HunkerDown))) return;
				// if the pawn is kidnaping a pawn skip.
				if (pawn.CurJobDef.Is(JobDefOf.Kidnap)) return;
				// Skip if some vanilla duties are active.
				var duty = pawn.mindState.duty;
				if (duty != null && (duty.def.Is(DutyDefOf.Build) || duty.def.Is(DutyDefOf.SleepForever) ||
				                     duty.def.Is(DutyDefOf.TravelOrLeave)))
				{
				...
				var warmup = (pawn.stances?.curStance ?? null) as Stance_Warmup;
				if (warmup != null && bodySize > 2.5f) return;
				...
				var fastCheck = false;
				if (warmup != null && (warmup.ticksLeft + GenTicks.TicksGame - warmup.startedTick > 120 ||
				                       warmup.ticksLeft < 30)) fastCheck = true;
				var verb = parent.TryGetAttackVerb();
				if (verb == null) return;
				if (verb.IsMeleeAttack)
				{
					if (pawn.CurJobDef == JobDefOf.Mine) pawn.jobs.StopAll();
					return;
				}

				if (!verb.Available() || (Mod_CE.active && Mod_CE.IsAimingCE(verb))) return;
				var bestEnemy = pawn.mindState.enemyTarget;
				var bestEnemyPositon = IntVec3.Invalid;
				var pawnPosition = pawn.Position;
				var bestEnemyScore = verb.currentTarget.IsValid && verb.currentTarget.Cell.IsValid
					? verb.currentTarget.Cell.DistanceToSquared(pawnPosition)
					: 1e6f;
				var bestEnemyVisibleNow = warmup != null;
				var retreat = false;
				var canRetreat = pawn.RaceProps.baseHealthScale <= 2.0f && pawn.RaceProps.baseBodySize <= 2.2f;
				var retreatDistSqr = Maths.Max(verb.EffectiveRange * verb.EffectiveRange / 9, 36);
				foreach (var enemy in visibleEnemies)
					if (enemy != null && enemy.Spawned && !enemy.Destroyed)
					{
						var shiftedPos = enemy.Position;
						var enemyPawn = enemy as Pawn;
						if (enemyPawn != null)
						{
							// skip for children
							var stage = enemyPawn.DevelopmentalStage;
							if (stage <= DevelopmentalStage.Child && stage != DevelopmentalStage.None) continue;
				...
				if (retreat)
				{
					...
					var request = new CoverPositionRequest();
					request.caster = pawn;
					...
					if (CoverPositionFinder.TryFindRetreatPosition(request, out var cell) && cell != pawnPosition)
					{
						var job_goto = JobMaker.MakeJob(JobDefOf.Goto, cell);
						job_goto.locomotionUrgency = LocomotionUrgency.Sprint;
						pawn.jobs.StartJob(moveJob = job_goto, JobCondition.InterruptForced);
					}
					else if (warmup == null)
					{
						var job_waitCombat = JobMaker.MakeJob(JobDefOf.Wait_Combat, Rand.Int % 100 + 100);
				...
				else if (!fastCheck)
				{
					...
					if (dist > 25)
					{
						if (bestEnemyVisibleNow)
						{
							...
							request.maxRangeFromCaster = Rand.Chance(Finder.P50 - 0.1f)
								? Mathf.Clamp(moveSpeed * 2 / (pawn.BodySize + 0.01f), 4, 10)
								: 4;
							request.wantCoverFromTarget = true;
							if (CastPositionFinder.TryFindCastPosition(request, out var cell) &&
							    (cell != pawnPosition || warmup == null))
							{
								var curDir = sightReader.GetEnemyDirection(cell).normalized;
								if (prevEnemyDir == Vector2.zero ||
								    Rand.Chance(Mathf.Abs(1 - Vector2.Dot(prevEnemyDir, curDir))) ||
								    Rand.Chance(sightReader.GetVisibilityToEnemies(pawn.Position) -
								                sightReader.GetVisibilityToEnemies(cell)))
								{
									var job_goto = JobMaker.MakeJob(JobDefOf.Goto, cell);
									job_goto.locomotionUrgency = LocomotionUrgency.Sprint;
									var job_waitCombat = JobMaker.MakeJob(JobDefOf.Wait_Combat, Rand.Int % 100 + 100);
						...
						else
						{
							...
							request.maxRangeFromCaster = Rand.Chance(Finder.P50 - 0.1f)
								...
								: 4;
							request.checkBlockChance = true;
							if (CoverPositionFinder.TryFindCoverPosition(request, out var cell) &&
							    (cell != pawnPosition || warmup == null))
							{
								var curDir = sightReader.GetEnemyDirection(cell).normalized;
								if (prevEnemyDir == Vector2.zero ||
		...
		public void Notify_TookDamage(DamageInfo dInfo)
		{
			...
			if (parent.Spawned && GenTicks.TicksGame - lastScanned < 90 && parent is Pawn pawn && !pawn.Dead &&
			    !pawn.Downed && armor.TankInt < 0.4f)
				if (dInfo.Def != null && dInfo.Instigator != null)
				{
					...
					else
					{
						...
						{
							var enemyRange = dInfo.Instigator.TryGetAttackVerb()?.EffectiveRange ?? 5f;
							var armorVal = armor.GetArmor(dInfo.Def);
							if (armorVal == 0 || Rand.Chance(dInfo.ArmorPenetrationInt / armorVal) ||
							    (GenTicks.TicksGame - lastTookDamage < 30 && Rand.Chance(0.50f)))
							{
								var pawnPosition = parent.Position;
								waitJob = null;
								...
								request.checkBlockChance = true;
								if (CoverPositionFinder.TryFindRetreatPosition(request, out var cell) &&
								    cell != pawnPosition)
								{
									var job_goto = JobMaker.MakeJob(JobDefOf.Goto, cell);
									job_goto.locomotionUrgency = LocomotionUrgency.Sprint;
		...
		public void Notify_EnemiesVisible(IEnumerable<Thing> things)
		{
			...
			{
				Log.Warning(
					$"ISMA: Notify_EnemiesVisible called while not scanning. ({visibleEnemies.Count}, {Thread.CurrentThread.ManagedThreadId})");
				return;
			}
		...
		public override void PostExposeData()
		{
			...
			if (parent is Pawn pawn)
			{
				if (duties == null) duties = new Pawn_CustomDutyTracker(pawn);
				duties.pawn = pawn;
			}
		}
		...
		public override void DrawGUIOverlay()
		{
			if (Finder.Settings.Debug && Finder.Settings.Debug_ValidateSight && parent is Pawn pawn)
			{
				...
				if (sightRange != 0 && verb != null)
				{
					...
					var shiftedPos = PawnPathUtility.GetMovingShiftedPosition(pawn, 30);
					var nearbyVisiblePawns = GenClosest.ThingsInRange(pawn.Position, pawn.Map,
							Utilities.TrackedThingsRequestCategory.Pawns, sightRange)
						.Select(t => t as Pawn)
						.Where(p => !p.Dead && !p.Downed &&
						            PawnPathUtility.GetMovingShiftedPosition(p, 60).DistanceToSquared(shiftedPos) <
						            sightRangeSqr &&
						            verb.CanHitTargetFrom(shiftedPos,
							            PawnPathUtility.GetMovingShiftedPosition(p, 60)) && p.HostileTo(pawn))
						.ToList();
					Gui.GUIUtility.ExecuteSafeGUIAction(() =>
					{
						var drawPosUI = UI.MapToUIPosition(drawPos);
						Text.Font = GameFont.Tiny;
						var state = GenTicks.TicksGame - lastInterupted > 120
							? "<color=blue>O</color>"
							: "<color=yellow>X</color>";
						Widgets.Label(new Rect(drawPosUI.x - 25, drawPosUI.y - 15, 50, 30),
							$"{state}/{_visibleEnemies.Count}");
					});
					var bugged = nearbyVisiblePawns.Count != _visibleEnemies.Count;
					if (bugged)
					{
						Rect rect;
						var a = UI.MapToUIPosition(drawPos);
						Vector2 b;
						Vector2 mid;
						foreach (var other in nearbyVisiblePawns.Where(p => !_visibleEnemies.Contains(p)))
						{
							...
							Widgets.Label(rect,
								$"<color=red>Errored</color>.  {Math.Round(other.Position.DistanceTo(pawn.Position), 1)}");
						}
					}

					var selected = Find.Selector.SelectedPawns.Contains(pawn);
					if (bugged || selected) GenDraw.DrawRadiusRing(pawn.Position, sightRange);
					if (selected)
					{
						for (var i = 1; i < _path.Count; i++)
						{
							Widgets.DrawBoxSolid(
								new Rect(
									UI.MapToUIPosition(_path[i - 1].ToVector3().Yto0() + new Vector3(0.5f, 0, 0.5f)) -
									new Vector2(5, 5), new Vector2(10, 10)), _colors[i]);
							Widgets.DrawLine(
								UI.MapToUIPosition(_path[i - 1].ToVector3().Yto0() + new Vector3(0.5f, 0, 0.5f)),
								UI.MapToUIPosition(_path[i].ToVector3().Yto0() + new Vector3(0.5f, 0, 0.5f)),
								Color.white, 1);
						}

						if (_path.Count > 0)
						{
							var v = UI.MapToUIPosition(pawn.DrawPos.Yto0());
							Widgets.DrawLine(
								UI.MapToUIPosition(_path.Last().ToVector3().Yto0() + new Vector3(0.5f, 0, 0.5f)), v,
								_colors.Last(), 1);
						...
						{
							var a = UI.MapToUIPosition(pawn.DrawPos);
							Vector2 b;
							Vector2 mid;
							Rect rect;
							var index = 0;
							foreach (var other in _visibleEnemies)
							{
								...
								Widgets.DrawBox(rect);
								Widgets.Label(rect,
									$"<color=gray>({index++}).</color> {Math.Round(other.Position.DistanceTo(pawn.Position), 1)}");
							}
						}
					}
				}
