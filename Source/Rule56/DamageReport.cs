...
namespace CombatAI
{
	...
	{
		private static Dictionary<ToolCapacityDef, List<ManeuverDef>> _maneuvers =
			new Dictionary<ToolCapacityDef, List<ManeuverDef>>(128);
		...
		/// </summary>		
		public bool IsValid => _finalized && GenTicks.TicksGame - _createdAt < 1800;
			...
			if (verb != null && verb.Available())
			{
				...
				if (!verb.IsMeleeAttack)
				{
					...
					if (projectile != null)
					{
						...
						var warmupTime = Maths.Max(verb.verbProps.warmupTime, 0.5f);
						float burstShotCount = verb.verbProps.burstShotCount;
						var output = 1f / warmupTime * burstShotCount;
						if (projectile.damageDef.armorCategory == DamageArmorCategoryDefOf.Sharp)
						{
							...
							rangedSharpAp = rangedSharpAp != 0
								? (rangedSharpAp + Maths.Max(GetArmorPenetration(projectile), 0f)) / 2f
								: Maths.Max(GetArmorPenetration(projectile), 0f);
						}
						else
						{
							rangedBlunt = (rangedBlunt + output * projectile.damageAmountBase) / 2f;
							rangedBluntAp = rangedBluntAp != 0
								? (rangedBluntAp + Maths.Max(GetArmorPenetration(projectile), 0f)) / 2f
								: Maths.Max(GetArmorPenetration(projectile), 0f);
						}
					}
				}
