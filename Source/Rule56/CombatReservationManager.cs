...
namespace CombatAI
{
	public class CombatReservationManager : MapComponent
	{
		private struct PawnReservationRecord : IExposable, IEquatable<PawnReservationRecord>
		{
			...
			public bool TargetIsPawn => _tragetIsPawn;
		...
		private class ReservedCombatTarget : IExposable
		{
			...
			public bool IsValid => target != null && target.Spawned;
		...
		{
			if (attacker != null && TryGetReservedCombatTarget(target, out var store))
				store.Add(attacker);
		}
		...
		{
			if (!TryGetReservedCombatTarget(target, out var store))
			{
				num = 0;
				return false;
			}
