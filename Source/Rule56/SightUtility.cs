...
namespace CombatAI
{
	public static class SightUtility
	{
		public static SightGrid.ISightRadius GetSightRadius(Thing thing)
		{
			...
			var sighter = thing.GetComp_Fast<ThingComp_Sighter>();
			if (sighter != null)
				...
				throw new Exception(
					$"ISMA: GetSightRadius got an object that is niether a pawn, turret nor does it have sighter. {thing}");
		...
		{
			var result = new SightGrid.ISightRadius();
			...
			var verb = pawn.equipment?.PrimaryEq?.PrimaryVerb ?? null;
			if (verb == null || !verb.Available())
				verb = pawn.verbTracker?.AllVerbs.Where(v => v.Available())
					.MaxBy(v => v.IsMeleeAttack ? 0 : v.EffectiveRange) ?? null;
			if (verb == null)
			{
				result.sight = 4;
				result.scan = 0;
			...
			if (verb.IsMeleeAttack)
			{
				var melee = pawn.skills?.GetSkill(SkillDefOf.Melee) ?? null;
				if (melee != null)
				{
					result.sight =
						Maths.Max(
							(int)(Mathf.Clamp(melee.Level, 5, 13) *
							      ((pawn.equipment?.Primary?.def.IsMeleeWeapon ?? null) != null ? 1.5f : 0.85f)), 15);
					result.scan = 0;
				}
				else
				{
			...
			else
			{
				...
				var shooting = pawn.skills?.GetSkill(SkillDefOf.Shooting) ?? null;
				float skill = 5;
				if (shooting != null) skill = shooting.Level;
