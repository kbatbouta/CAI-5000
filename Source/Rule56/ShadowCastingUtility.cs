...
namespace CombatAI
{
	...
	public static partial class ShadowCastingUtility
	{
		...
		static ShadowCastingUtility()
		{
			...
			_transformationFuncs[0] = (cell) => { return cell; };
			_transformationFuncs[1] = (cell) => { return new IntVec3(cell.z, 0, -1 * cell.x); };
			_transformationFuncs[2] = (cell) => { return new IntVec3(-1 * cell.x, 0, -1 * cell.z); };
			_transformationFuncs[3] = (cell) => { return new IntVec3(-1 * cell.z, 0, cell.x); };
			_transformationInverseFuncs = new Func<IntVec3, IntVec3>[4];
			_transformationInverseFuncs[0] = (cell) => { return cell; };
			_transformationInverseFuncs[1] = (cell) => { return new IntVec3(-1 * cell.z, 0, cell.x); };
			_transformationInverseFuncs[2] = (cell) => { return new IntVec3(-1 * cell.x, 0, -1 * cell.z); };
			_transformationInverseFuncs[3] = (cell) => { return new IntVec3(cell.z, 0, -1 * cell.x); };
			_transformationFuncsV3 = new Func<Vector3, Vector3>[4];
			_transformationFuncsV3[0] = (cell) => { return cell; };
			_transformationFuncsV3[1] = (cell) => { return new Vector3(cell.z, 0, -1 * cell.x); };
			_transformationFuncsV3[2] = (cell) => { return new Vector3(-1 * cell.x, 0, -1 * cell.z); };
			_transformationFuncsV3[3] = (cell) => { return new Vector3(-1 * cell.z, 0, cell.x); };
			_transformationInverseFuncsV3 = new Func<Vector3, Vector3>[4];
			_transformationInverseFuncsV3[0] = (cell) => { return cell; };
			_transformationInverseFuncsV3[1] = (cell) => { return new Vector3(-1 * cell.z, 0, cell.x); };
			_transformationInverseFuncsV3[2] = (cell) => { return new Vector3(-1 * cell.x, 0, -1 * cell.z); };
			_transformationInverseFuncsV3[3] = (cell) => { return new Vector3(cell.z, 0, -1 * cell.x); };
		}
		...
		private struct VisibleRow
		{
			...
			{
				var min = (int)Mathf.Floor(startSlope * depth + 0.5f);
				var max = (int)Mathf.Ceil(endSlope * depth - 0.5f);

				buffer.Clear();
				for (var i = min; i <= max; i++) buffer.Add(new Vector3(depth, 0, i));
			}

			public void DebugFlash(Map map, IntVec3 root)
			{
				var startSlope = this.startSlope;
				var endSlope = this.endSlope;
				var min = (int)Mathf.Floor(startSlope * this.depth + 0.5f);
				var max = (int)Mathf.Ceil(endSlope * this.depth - 0.5f);
				var left = root + _transformationFuncs[quartor](new IntVec3(this.depth, 0, min));
				var right = root + _transformationFuncs[quartor](new IntVec3(this.depth, 0, max));
				var depth = this.depth;
				map.GetComponent<MapComponent_CombatAI>().EnqueueMainThreadAction(() =>
				{
					if (left.InBounds(map)) map.debugDrawer.FlashCell(left, 0.99f, $"{Math.Round(startSlope, 3)}");
					if (right.InBounds(map)) map.debugDrawer.FlashCell(right, 0.01f, $"{Math.Round(endSlope, 3)}");
				});
			}
		...
		private static int GetQurator(IntVec3 cell)
		{
			...
			if (x > 0 && Math.Abs(z) < x)
				...
				return 2;
			else if (z > 0 && Math.Abs(x) <= z) return 3;
			return 1;
		}
		...
		private static void TryWeightedScan(CastRequest request)
		{
			...
			var rowQueue = new List<VisibleRow>();
			rowQueue.Add(request.firstRow);
			var coverMinDist = Maths.Min(Maths.Max(request.firstRow.maxDepth / 3f, 4f), 8f);
			var coverBlockInc = 1f / Maths.Max(request.firstRow.maxDepth, 1);
			while (rowQueue.Count > 0)
			{
				VisibleRow nextRow;
				var row = rowQueue.Pop();

				if (row.depth > row.maxDepth || row.visibilityCarry <= 1e-5f) continue;
				...
				//row.DebugFlash(request.map, request.source);                
				for (var i = 0; i < request.buffer.Count; i++)
				{
					var offset = request.buffer[i];
					var fill = FillCategory.None;
					var cell = request.source + row.Transform(offset.ToIntVec3());
					var isWall = !cell.InBounds(request.map) ||
					             (fill = grid.GetFillCategory(cell)) == FillCategory.Full;
					var process = true;
					if (!isWall && (i == 0 || i - lastNextIndex == 1 || lastIsWall))
					{
						var temp = request.source + row.Transform(offset.ToIntVec3() + Back);
						if (temp.InBounds(request.map) && grid.GetFillCategory(temp) == FillCategory.Full)
						{
							temp = request.source +
							       row.Transform(offset.ToIntVec3() + new IntVec3(0, 0, offset.z < 0 ? 1 : -1));
							if (temp.InBounds(request.map) && grid.GetFillCategory(temp) == FillCategory.Full)
							{
								isWall = true;
								process = false;
					...
					//	//}
					//});
					var isCover = !isWall && fill == FillCategory.Partial;

					if (process && (isWall ||
					                (offset.z >= row.depth * row.startSlope && offset.z <= row.depth * row.endSlope)))
						...
						if (lastIsWall == isWall)
						{
							if (isCover != lastIsCover) // first handle cover 
							{
								...
								if (row.depth > coverMinDist)
								{
									if (lastIsCover)
									{
										nextRow.blockChance += Maths.Min((1 - row.blockChance) * lastFill / lastFillNum,
											1.0f);
		...
		private static void TryVisibilityScan(CastRequest request)
		{
			...
			while (rowQueue.Count > 0)
			{
				...
				if (row.depth > row.maxDepth) continue;
				var lastCell = InvalidOffset;
				var lastIsWall = false;
				var lastNextIndex = 0;
				row.Tiles(request.buffer);
				...
				if (lastCell.y >= 0 && !lastIsWall) rowQueue.Add(row.Next());
			}
		}

		private static void TryCastVisibility(float startSlope, float endSlope, int quartor, int maxDepth,
			int carryLimit, IntVec3 source, Map map, Action<IntVec3, int, int, float> setAction, List<Vector3> buffer)
		{
			TryCast(TryVisibilityScan, startSlope, endSlope, quartor, maxDepth, carryLimit, source, map, setAction,
				buffer);
		}

		private static void TryCastWeighted(float startSlope, float endSlope, int quartor, int maxDepth, int carryLimit,
			IntVec3 source, Map map, Action<IntVec3, int, int, float> setAction, List<Vector3> buffer)
		{
			TryCast(TryWeightedScan, startSlope, endSlope, quartor, maxDepth, carryLimit, source, map, setAction,
				buffer);
		}

		private static void TryCast(Action<CastRequest> castAction, float startSlope, float endSlope, int quartor,
			int maxDepth, int carryLimit, IntVec3 source, Map map, Action<IntVec3, int, int, float> setAction,
			List<Vector3> buffer)
		{
			if (endSlope > 1.0f || startSlope < -1 || startSlope > endSlope)
				throw new InvalidOperationException(
					$"CE: Scan quartor {quartor} endSlope and start slop must be between (-1, 1) but got start:{startSlope}\tend:{endSlope}");
			var grid = map.GetComponent<WallGrid>();
			if (grid == null)
			{
			...
			buffer.Clear();
			var coverLoc = source + _transformationFuncs[quartor](new IntVec3(1, 0, 0));
			if (coverLoc.InBounds(map) && grid.GetFillCategory(coverLoc) == FillCategory.Full)
			{
				var rightUpper = source + _transformationFuncs[quartor](new IntVec3(1, 0, -1));
				var rightLower = source + _transformationFuncs[quartor](new IntVec3(0, 0, -1));
				var rightBlocked = !rightUpper.InBounds(map) || grid.GetFillCategory(rightUpper) == FillCategory.Full ||
				                   !rightLower.InBounds(map) || grid.GetFillCategory(rightLower) == FillCategory.Full;
				var leftUpper = source + _transformationFuncs[quartor](new IntVec3(1, 0, 1));
				var leftLower = source + _transformationFuncs[quartor](new IntVec3(0, 0, 1));
				var leftBlocked = !leftUpper.InBounds(map) || grid.GetFillCategory(leftUpper) == FillCategory.Full ||
				                  !leftLower.InBounds(map) || grid.GetFillCategory(leftLower) == FillCategory.Full;
				if (rightBlocked != leftBlocked || (!leftBlocked && !rightBlocked))
				{
					if (!leftBlocked)
					{
						var requestLeft = new CastRequest();
						var arcLeft = VisibleRow.First;
						arcLeft.startSlope =
							!Finder.Settings.LeanCE_Enabled ? startSlope : -1f / Maths.Max(maxDepth, 64);
					...
					if (!rightBlocked)
					{
						var requestRight = new CastRequest();
						var arcRight = VisibleRow.First;
			...
			{
				var request = new CastRequest();
				var arc = VisibleRow.First;
