...
namespace CombatAI.Patches
{
    public static class MapPawns_Patch
    {
        ...
        {
            public static void Prefix(MapPawns __instance, Pawn p) => __instance.map.GetComp_Fast<SightTracker>().Register(p);
        }

        [HarmonyPatch(typeof(MapPawns), nameof(MapPawns.DeRegisterPawn))]
        public static class MapPawns_DeRegisterPawn_Patch
        {
            public static void Prefix(MapPawns __instance, Pawn p) => __instance.map.GetComp_Fast<SightTracker>().DeRegister(p);
        }
    }
}
